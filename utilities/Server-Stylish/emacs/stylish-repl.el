...
(defun* stylish-handler-repl (&key status result stdout stderr)
  "Handle a return from the REPL"
  (with-current-buffer (get-buffer +stylish-repl-buffer)
    (save-excursion
      (goto-char (point-max))
...
(defun stylish-repl-eval-perl (perl)
  (stylish-send-command 'repl :command perl))
...
;;  (interactive)
;;  (let ((bounds (stylish-repl-input-region-bounds)))
;;    (stylish-syntaxify (car bounds) (cdr bounds) (get-buffer +stylish-repl-buffer))))
;;
