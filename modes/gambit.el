...
;; Copyright (c) 1997-1998 Marc Feeley & Michael Sperber
...
(defvar scheme-program-name "gsi -:t") ; "-:t" needed for Windows95/NT
...
(defvar gambit-create-new-frame nil
  "Flag to create a new frame to highlight a Scheme expression.")

(defvar gambit-keep-selected-on-top t
  "Flag to keep selected window on top.")

(defvar gambit-unhighlight-after-command t
  "Flag to remove highlighting of a Scheme expression after a command is entered.")

(defvar gambit-duplicate-scheme-buffer nil
  "Flag to allow Scheme buffer to be duplicated to highlight a Scheme expression.")

(defvar gambit-new-window-height 6
  "Height of a window opened to highlight a Scheme expression.")

(defvar gambit-max-popups 1
  "Maximum number of popup windows opened to highlight a Scheme expression.")
...
(defun gambit-input-sender (proc str)
  (let ((clean-str (gambit-string-terminate-with-newline str)))
    ...
    (if gambit-unhighlight-after-command
        (gambit-unhighlight))
...
(defun gambit-trim-popups ()
  (setq gambit-popups (gambit-gc-popups gambit-popups))
  (while (>= (length gambit-popups) gambit-max-popups)
    (gambit-kill-last-popup)))

(defun gambit-add-popup (popup)
  (setq gambit-popups (cons popup gambit-popups)))
...
enlarge the window if it is too small."
  ...
  (let* ((location-windows
    ...
    (if (or (null location-windows)
            (and gambit-duplicate-scheme-buffer
                 (eq location-buffer (get-buffer scheme-buffer))
                 (eq initially-selected-window (car location-windows))
                 (null (cdr location-windows))))

        ; the buffer is not visible or it is the Scheme buffer and only
        ; visible in one window

        (if gambit-create-new-frame

            (switch-to-buffer-other-frame location-buffer)

            (progn
              (gambit-trim-popups)
              (let* ((scheme-windows ; list of windows displaying Scheme buffer
                      (gambit-windows-displaying-buffer
                       (get-buffer scheme-buffer)))
                     (window-to-split ; the window that will get split
    ...
    (if (not (eq initially-selected-window (selected-window)))
        (progn
          ...
          (if (not gambit-move-to-highlighted)
              (select-window initially-selected-window))
          (if gambit-keep-selected-on-top
              (raise-frame (window-frame initially-selected-window)))))))
...
(defun gambit-inferior-mode ()
  ...
  (set-marker gambit-last-output-marker (point))
